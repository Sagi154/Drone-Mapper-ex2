...
types::SimulationManagerReport SimulationManager::run(const types::SimulationCompositionData& composition,
    ...
    const std::size_t pair_count = composition.simulations.size();
    if (pair_count != composition.missions.size()) {
        throw std::invalid_argument(
            "SimulationCompositionData requires aligned simulations[] and missions[] pair vectors.");
    }

    yaml_entries.reserve(pair_count * composition.drones.size() * composition.lidars.size());

    int run_id = 1;
    for (std::size_t pair_index = 0; pair_index < pair_count; ++pair_index) {
        const types::SimulationConfigData& simulation = composition.simulations[pair_index];
        const types::MissionConfigData& mission = composition.missions[pair_index];
